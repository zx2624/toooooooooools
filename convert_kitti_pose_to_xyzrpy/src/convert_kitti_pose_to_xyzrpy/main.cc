...
int main(int argc, char **argv) {
	...
	//kitti is rotate of fixed axis x, y, z
	auto rpy = camera_to_lidar.block<3, 3>(0, 0).eulerAngles(2, 0, 2);
	roll = rpy(2);
	pitch = rpy(1);
	yaw = rpy(0);
	...
	while(ifs >> 
		...
		Matrix4d lidar = camera_to_lidar.inverse() * cam * camera_to_lidar;
		//Matrix4d lidar = cam;
		double roll = 0, pitch = 0, yaw = 0;
		...
		//auto rpy = lidar.block<3, 3>(0, 0).eulerAngles(2, 1, 0);
		//roll = rpy(0);
		//pitch = rpy(1);
		//yaw = rpy(2);
