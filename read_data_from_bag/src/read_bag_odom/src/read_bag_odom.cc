...
void ReadGNSS::SaveOdomToFile() {
  ...
  for (auto bag_name : names) {
    ...
    for (rosbag::View::iterator its = gnss_odom_view.begin();
      ...
      tf::Matrix3x3(tf::Quaternion(od->pose.pose.orientation.x,
      ...
      ofs_raw
          << std::fixed << std::setprecision(6) << od->header.stamp.toSec()
          << " " << od->pose.pose.position.x - init_x << " "
          << od->pose.pose.position.y - init_y << " "
          << od->pose.pose.position.z - init_z << " " << rpy_raw(0) << " "
          << rpy_raw(1) << " "
          << rpy_raw(2)
          //<< " " << rpy_debug(2) << " " << rpy_debug(1) << " " << rpy_debug(0)
          << " " << covariance[0] << " " << covariance[1] << " "
          << covariance[2] << " " << covariance[3] << " " << covariance[4]
          << " " << covariance[5] << std::endl;

      cnt++;

      if (covariance[0] > 2 || covariance[1] > 2 || covariance[2] > 10 ||
          covariance[3] > 2 || covariance[4] > 2 || covariance[5] > 2) {
        continue;
      }

      ofs << std::fixed << std::setprecision(6) << od->header.stamp.toSec()
          << " " << od->pose.pose.position.x - init_x << " "
          << od->pose.pose.position.y - init_y << " "
          << od->pose.pose.position.z - init_z << " " << rpy_raw(0) << " "
          << rpy_raw(1) << " " << rpy_raw(2) << " " << covariance[0] << " "
